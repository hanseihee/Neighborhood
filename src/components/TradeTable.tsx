...
export default function TradeTable({ trades, rents = [], mode = 'trade', initialRows = 20 }: TradeTableProps) {
  const [showAll, setShowAll] = useState(false);
  const [sortKey, setSortKey] = useState<SortKey>('date');
  const [sortDir, setSortDir] = useState<SortDir>('desc');
  const [selectedTrade, setSelectedTrade] = useState<AptTrade | null>(null);
  const [selectedRent, setSelectedRent] = useState<AptRent | null>(null);

  const isRent = mode === 'rent';

  // rent 모드
  if (isRent) {
    const maxDeposit = rents.length > 0 ? Math.max(...rents.map(r => r.보증금)) : 0;

    const sortedRents = [...rents].sort((a, b) => {
      const dir = sortDir === 'desc' ? -1 : 1;
      switch (sortKey) {
        case 'dealAmount':
          return (a.보증금 - b.보증금) * dir;
        case 'area':
    ...
    const displayedRents = showAll ? sortedRents : sortedRents.slice(0, initialRows);
    ...
    return (
      <div>
                ...
                >
                  ...
                  <td className="py-3 px-3 text-right">
                    <span className="inline-flex items-center gap-1.5">
                      {rent.보증금 === maxDeposit && rents.length > 1 && (
                        <span className="px-1.5 py-0.5 rounded text-[11px] font-semibold bg-red-50 text-red-500">
                          최고
                        </span>
                      )}
                      <span className={`text-[15px] font-bold tabular-nums ${
                        rent.보증금 === maxDeposit && rents.length > 1 ? 'text-red-500' : 'text-slate-900'
                      }`}>
                        {formatPrice(rent.보증금)}
                      </span>
                    </span>
                  </td>
                  <td className="py-3 px-3 text-right text-[13px] tabular-nums hidden lg:table-cell">
                    {rent.월세 > 0 ? (
                      <span className="text-amber-600 font-medium">{rent.월세}만</span>
                    ) : (
        ...
        {rents.length > initialRows && !showAll && (
          <button
...
function TradeDetailModal({
  ...
  return (
    <DetailModalShell title="거래 상세 정보" rows={rows} onClose={onClose} />
  );
}

function RentDetailModal({
  rent,
  onClose,
}: {
  rent: AptRent;
  ...
  const rows: { label: string; value: string; highlight?: boolean }[] = [
    { label: '단지명', value: rent.아파트, highlight: true },
    { label: '보증금', value: formatPrice(rent.보증금), highlight: true },
    { label: '월세', value: rent.월세 > 0 ? `${rent.월세}만원` : '전세', highlight: true },
    { label: '계약일', value: formatDate(rent.년, rent.월, rent.일) },
    { label: '전용면적', value: formatArea(rent.전용면적) },
    { label: '층', value: `${rent.층}층` },
    { label: '건축년도', value: rent.건축년도 ? `${rent.건축년도}년` : '-' },
    { label: '법정동', value: rent.법정동 || '-' },
    { label: '지번', value: rent.지번 || '-' },
    { label: '계약기간', value: rent.계약기간 || '-' },
    { label: '계약구분', value: rent.계약구분 || '-' },
    { label: '갱신요구권', value: rent.갱신요구권사용 || '-' },
    { label: '종전 보증금', value: rent.종전보증금 != null ? formatPrice(rent.종전보증금) : '-' },
    { label: '종전 월세', value: rent.종전월세 != null ? `${rent.종전월세}만원` : '-' },
  ];

  return (
    <DetailModalShell title="전월세 상세 정보" rows={rows} onClose={onClose} />
  );
}
