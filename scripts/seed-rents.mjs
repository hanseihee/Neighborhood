/**
 * MOLIT 전월세 API → Supabase 데이터 적재 스크립트
 * Usage: node scripts/seed-rents.mjs [months=36]
 */
...
const MOLIT_BASE = 'https://apis.data.go.kr/1613000/RTMSDataSvcAptRent/getRTMSDataSvcAptRent';
...
function parseXml(xml, lawdCd) {
  ...
  return items
    .map(item => {
      ...
      const deposit = parseInt(get('deposit').replace(/,/g, ''));
      if (isNaN(deposit) || deposit <= 0) return null;
      const monthlyRent = parseInt(get('monthlyRent').replace(/,/g, '')) || 0;
      return {
        ...
        contract_term: get('contractTerm') || null,
        contract_type: get('contractType') || null,
        use_rr_right: get('useRRRight') || null,
        pre_deposit: parseInt(get('preDeposit').replace(/,/g, '')) || null,
        pre_monthly_rent: parseInt(get('preMonthlyRent').replace(/,/g, '')) || null,
      };
    })
    .filter(Boolean);
}
...
async function upsertToSupabase(rows) {
  ...
  for (let i = 0; i < rows.length; i += 500) {
    ...
    const res = await fetch(`${SUPABASE_URL}/rest/v1/apt_rents?on_conflict=apartment_name,deal_year,deal_month,deal_day,exclusive_area,floor,district_code,deposit`, {
      method: 'POST',
      headers: {
        'apikey': SUPABASE_KEY,
...
/** Materialized View 새로고침 (Supabase RPC) */
async function refreshViews() {
  console.log('\n=== Materialized View 새로고침 ===');
  const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/refresh_rent_views`, {
    method: 'POST',
    headers: {
      'apikey': SUPABASE_KEY,
      'Authorization': `Bearer ${SUPABASE_KEY}`,
  ...
  if (!res.ok) {
    const err = await res.text();
    console.error(`  View 새로고침 실패: ${res.status} ${err.substring(0, 300)}`);
    return false;
  }
  console.log('  rent_metro_monthly_summary ✓');
  console.log('  rent_district_monthly_summary ✓');
  console.log('  rent_apartment_search ✓');
  return true;
}
...
async function main() {
  ...
  for (let di = 0; di < DISTRICTS.length; di++) {
    ...
    for (let mi = 0; mi < monthList.length; mi += 6) {
      ...
      if (allRents.length > 0) {
        const inserted = await upsertToSupabase(allRents);
        distRows += inserted;
      }
      // API rate limit 방지
      await sleep(200);
  ...
  // 데이터 적재 후 Materialized View 새로고침
  await refreshViews();
